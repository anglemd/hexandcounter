...
export class GameInfoComponent implements OnInit {
  ...
  constructor(private gameService: GameService, private routerService: RouterService, private route: Router) {
    this.currentGame$ = this.gameService.currentGame$;

    this.route.events.pipe(filter((evt) => evt instanceof NavigationStart)).subscribe((evt) => {
